...
const SECTIONS = [
  ...
  { id: 'history', label: '현병력', status: 'done' },
  { id: 'past', label: '과거력', status: 'empty' },
...
const NewCasePage = ({ setCurrentView}) => {
  ...
  const [diagnoses, setDiagnoses] = useState([
    { id: 1, title: '비효율적 기도 청결', related: '점액 과다 분비' },
    { id: 2, title: '체액 부족 위험성', related: '설사 및 고열' }
  ]); //간호 문제
  ...
  const toggleDisease = (disease) => {
   ...
   const [labTests, setLabTests] = useState([
    { id: 1, item: 'WBC', val: '0.0', unit: '10³/μL', abnormal: false },
    { id: 2, item: 'Hb', val: '0.0', unit: 'g/dL', abnormal: false }
   ]);
   ...
   //간호 기록
   useEffect(() => {
    const pendingId = localStorage.getItem('pending-log-id');

    if(pendingId) {
        const targetLog = allLogs.find(l => l.id === pendingId === pendingId.toString());

        if(targetLog){
            loadLogToEditor(targetLog.id);
        }

        localStorage.removeItem('pending-log-id');
    }
   }, [allLogs]);
   ...
   const loadLogToEditor = (logId) => {
    ...
    const openLogFromArchive = (log) => {
    handleLoadLogForEdit(log); 

    setCurrentView('dashboard'); 
    };
  return (
    <div className="md:hidden sticky top-0 w-full z-30 bg-white/80 backdrop-blur-md border-b border-slate-100">
        ...
        className={`${
            isMenuOpen 
            ? 'fixed inset-0 z-50 bg-white flex flex-col p-6 w-full'
            : 'hidden md:flex'
        } md:w-64 border-r border-slate-200/60 p-4 flex-col bg-white shrink-0 md:h-full`}
        >
        {/* 모바일 닫기 버튼 */}
        {isMenuOpen && (
      ...
      {/* 우측 작성 영역 */}
      <main className="flex-1 overflow-y-auto p-4 md:p-8 pt-0 md:pt-8 pb-24">
        <div className="max-w-3xl mx-auto space-y-4">
            ...
            <div className="space-y-4">
                ...
                <textarea 
                    ...
                    placeholder="증상의 시작 시점, 양상, 빈도, 악화/완화 요인을 기술하세요 (예: 2일 전부터 시작된..."
                />
                </div>
            ...
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-sm font-black text-slate-800 flex items-center gap-2">
                <span className="w-1 h-3 bg-indigo-500 rounded-full"></span> 8. 실습 기록
                </h2>
                <div className="mb-4">
                <label className="text-[10px] font-bold text-slate-400">일지 불러오기</label>
                <select 
