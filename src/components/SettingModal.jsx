    ...
    <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm z-30 flex flex-col justify-end text-xs">
      <div className="bg-white max-h-[90vh] rounded-t-2xl p-4 flex flex-col justify-between shadow-2xl overflow-hidden">
        ...
        <div className="flex justify-between items-center border-b pb-3 mb-3">
          <div>
            <h2 className="text-base font-bold text-gray-800">⚙️ 설정</h2>
          </div>
          <button type="button" onClick={() => setIsSettingMode(false)} className="text-gray-400 hover:text-gray-600 font-bold p-1 text-sm">✕ 닫기</button>
        </div>
        ...
        <div className="flex border-b mb-3 text-xs font-bold text-center">
          <button 
            ...
            className={`flex-1 py-2 border-b-2 ${activeSettingTab === 'routine' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-400'}`}
          >
            ⏱️ 루틴 편집
          </button>
          <button 
            ...
            className={`flex-1 py-2 border-b-2 ${activeSettingTab === 'tags' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-400'}`}
          >
            🏷️ 태그 / 용어 편집
          </button>
        </div>
          ...
          {activeSettingTab === 'routine' ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 border-t pt-3">
                ...
                <button type="button" onClick={handleSaveCustomSkeletons} className="bg-indigo-600 text-white font-bold p-2 rounded-xl text-xs shadow hover:bg-indigo-700">💾 루틴 저장</button>
              </div>
            </div>
          ) : (
            /* 태그 및 의학용어 편집 */
            <div className="space-y-4">
              <div className="space-y-2 bg-slate-50/70 p-3 rounded-xl border border-gray-200">
                <span className="font-bold text-gray-700 block text-xs">🏷️ 실습 상용구 태그 및 NANDA 연동 등록</span>
                <div className="space-y-2 bg-white p-2.5 rounded-lg border">
                  ...
                  <button type="button" onClick={handleAddCustomTag} className="w-full bg-indigo-600 text-white font-bold p-2 rounded-lg text-xs shadow-sm hover:bg-indigo-700">➕ 태그 추가 등록</button>
                </div>
              ...
              <div className="space-y-2 bg-slate-50/50 p-2.5 rounded-xl border">
                <span className="font-bold text-gray-700 block">🔍 약어 사전 등록</span>
              ...
              <button type="button" onClick={handleSaveTagsAndDict} className="w-full bg-slate-900 text-white font-bold p-3 rounded-xl text-xs shadow-md hover:bg-slate-800 transition">
                💾 저장
              </button>
            </div>
          )}
